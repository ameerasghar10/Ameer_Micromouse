...
# Robot wheelbase width for turning calculations
robot_width_mm = 80  # Updated to your robot's actual wheelbase width in mm
turn_calibration_factor = 0.5  # Adjusted based on testing
...
def turn_left_90():
    ...
    # Calculate counts needed for 90-degree turn
    ...
    # Set motors to turn left in place (left motor backward, right motor forward)
    set_motor('left', 0, base_speed_left)  # Left motor backward (direction 0)
    set_motor('right', direction_right, base_speed_right)  # Right motor forward (direction 0)
    ...
    while True:
        ...
        # Check if counts have reached turn_counts
...
try:
    while True:
        ...
        if distance_front <= stop_distance and distance_front != 0:
            ...
            turn_left_90()
            break  # Exit the loop after completing the action
